        ...
        def start_training():
            ...
            for idx, item in enumerate(texts):
                ...
                if label in target_counts:
                    if current_counts.get(label, 0) < target_counts[label]:
                        selected_indices.append(idx)
                        current_counts[label] = current_counts.get(label, 0) + 1


            class REGDataset(Dataset):
                def __init__(self, patches_mmap, texts: list[str]):
                    self.patches_mmap = patches_mmap
                    self.texts = texts
                    self.selected_indices = selected_indices

                def __len__(self):
                    return len(self.selected_indices)-4
            ...
            train_data = REGDataset(patches, texts)
            ...
            class Model(nn.Module):
                def __init__(self, image_model, language_model, tokenizer, prompt="Describe the medical image:"):
                    ...
                    start_token_tensor = torch.tensor([[self.start_token_id]], device="cuda:0")
                    end_token_tensor = torch.tensor([[self.end_token_id]], device="cuda:0")
                    
                    start_embedding = language_model.get_input_embeddings()(start_token_tensor).detach()
                    end_embedding = language_model.get_input_embeddings()(end_token_tensor).detach()
                    
                    self.register_buffer('start_embedding', start_embedding)
                    self.register_buffer('end_embedding', end_embedding)
                ...
                def forward(self, patches: torch.Tensor, texts: List[str]):
                    ...
                    # Add <end> token to each text
                    texts_with_end = [text + " <end>" + self.tokenizer.eos_token for text in texts]
                    ...
                    # inputs are embeddings of -> prompt + image patches + <start> + text + <end>
                    embeddings = torch.cat([
                        self.prompt_embeddings.to(device).expand(patches.size(0), -1, -1),
                        patch_embeddings,
                        self.start_embedding.to(device).expand(patches.size(0), -1, -1),
                        text_embeddings,
                    ], dim=1)
                    ...
                    start_mask = torch.ones(patches.size(0), 1, device=device)  # <start> token mask
                    attention_mask = torch.cat([prompt_mask, patch_mask, start_mask, tokenized["attention_mask"]], dim=1)
                    ...
                    start_labels = torch.full((patches.size(0), 1), CFG.LABEL_MASK, device=device)  # Don't predict <start>
                    text_labels = tokenized["input_ids"].clone()
                    labels = torch.cat([prompt_labels, patch_labels, start_labels, text_labels], dim=1)
                ...
                def generate(self, patches: torch.Tensor, generator_kwargs: dict[str, Union[int, float]]):
                    ...
                    # Use the same processing as in forward method
                    image_features = self.image_model.forward_features(patches)
                    image_features = einops.rearrange(image_features, "bs dim w h -> bs (w h) dim")
                    patch_embeddings = self.projector(image_features)
                    
                    # embeddings = prompt + image patches + <start>
                    embeddings = torch.cat([
                        self.prompt_embeddings.to(device).expand(patches.size(0), -1, -1),
                        patch_embeddings,
                        self.start_embedding.to(device).expand(patches.size(0), -1, -1),
                    ...
                    prompt_mask = torch.ones(patches.size(0), self.prompt_embeddings.size(1), device=device)
                    patch_mask = torch.ones(patches.size(0), patch_embeddings.size(1), device=device)
                    start_mask = torch.ones(patches.size(0), 1, device=device)
                    attention_mask = torch.cat([prompt_mask, patch_mask, start_mask], dim=1)
                    
                    # Set the end token as eos_token_id for stopping generation
                    generator_kwargs = generator_kwargs.copy()
                    generator_kwargs['eos_token_id'] = self.end_token_id
            ...
            # patches_batch = np.array(patches[720:730])
            patches_batch = np.array([patches[i] for i in [689, 694, 696]])

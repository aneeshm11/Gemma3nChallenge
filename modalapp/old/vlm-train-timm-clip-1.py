        ...
        def start_training():
            ...
            class Model(nn.Module):
                def __init__(self, image_model, language_model, tokenizer, prompt="Describe the medical image:"):
                    ...
                    # Projection for sequence modeling (patch-level)
                    self.projector = nn.Sequential(
                        *projection_layers(image_model.num_features, 2048, 6)
                    ).to(device)
                    
                    # Global pooling and projections for contrastive learning
                    self.global_pool = nn.AdaptiveAvgPool2d(1)
                    self.image_projection = nn.Linear(image_model.num_features, 2048).to(device)
                    self.text_projection = nn.Linear(2048, 2048).to(device)
                ...
                def forward(self, patches: torch.Tensor, texts: List[str]):
                    ...
                    # Global image representation for contrastive learning
                    global_image_features = self.global_pool(image_features).flatten(1)
                    global_image_embed = self.image_projection(global_image_features)
                    
                    # Patch-level features for sequence modeling
                    image_features_seq = einops.rearrange(image_features, "bs dim w h -> bs (w h) dim")
                    patch_embeddings = self.projector(image_features_seq)
                    ...
                    # Global text representation for contrastive learning (mean pooling)
                    text_mask = tokenized["attention_mask"].unsqueeze(-1).float()
                    global_text_embed = (text_embeddings * text_mask).sum(dim=1) / text_mask.sum(dim=1)
                    global_text_embed = self.text_projection(global_text_embed)

                    # Concatenate embeddings for sequence modeling: prompt + image patches + text
                    ...
                    # labels - only compute loss on text tokens
                    ...
                    text_labels[tokenized["attention_mask"] == 0] = CFG.LABEL_MASK  # mask padding
                    ...
                    contrastive_loss = self.compute_contrastive_loss(global_image_embed, global_text_embed)
                    
                    # Combine losses (weighted)
                    total_loss = lm_outputs.loss + 0.1 * contrastive_loss
            ...
            class LightningModule(LIGHTNING.LightningModule):
                ...
                def training_step(self, batch, batch_idx):
                    ...
                    self.log("generation_loss", outputs.generation_loss)  
                    self.log("contrastive_loss", outputs.contrastive_loss)
                ...
                def configure_optimizers(self) -> torch.optim.Optimizer:
                    params = [
                        ...
                        {"params": self.model.image_projection.parameters(), "lr": self.lr},
            ...
            class PrintLossCallback(Callback):
                def on_train_epoch_end(self, trainer, pl_module):
                    ...
                    gen_loss = trainer.callback_metrics.get("generation_loss")
                    cont_loss = trainer.callback_metrics.get("contrastive_loss")
                    
                    if train_loss is not None:
                        print(f"Epoch {trainer.current_epoch} - Train Loss: {train_loss.item():.4f}")
            ...
            patches_batch = np.array(patches[-10:])  # Changed from 50 to 10
            patches_batch = torch.tensor(patches_batch)
            print(f"Inference batch shape: {patches_batch.shape}")
            ...
            for i in tqdm(range(generated_ids.shape[0])):
                ...
                patch_length = patches_batch.shape[1] * patches_batch.shape[2]  # w * h from einops rearrange
            ...
            torch.save(lightning_module.model.image_projection.state_dict(), str(trained_vision_model_path / "image_projection.pth"))
            torch.save(lightning_module.model.text_projection.state_dict(), str(trained_vision_model_path / "text_projection.pth"))

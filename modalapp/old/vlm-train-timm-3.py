...
class CFG:
    ...
    MAX_LENGTH = 50
    LABEL_MASK = -100  # for loss calculation - this masks tokens we don't want to compute loss on
...
def train_and_upload():
    ...
    print("STARTING MASKED LOSS TRAINING WORKFLOW...")    
    ...
    # ----------------------------------------- TRAINING WORKFLOW ------------------------------------------
        ...
        def start_training():
            ...
            class MaskedLossModel(nn.Module):
                def __init__(self, image_model, language_model, tokenizer, prompt="Describe the medical image:"):
                    ...
                    # Projector to map image features to LLM embedding dimension
                ...
                def forward(self, patches: torch.Tensor, texts: List[str]):
                    ...
                    # Extract image features and project to LLM embedding space
                    ...
                    # Concatenate embeddings: [prompt, image_patches, text]
                    ...
                    # *** KEY CHANGE: Create labels that ONLY compute loss on text tokens ***
                    # Mask prompt tokens (set to -100 so loss is ignored)
                    prompt_labels = torch.full((patches.size(0), self.prompt_embeddings.size(1)), CFG.LABEL_MASK, device=device)
                    
                    # Mask image patch tokens (set to -100 so loss is ignored) 
                    patch_labels = torch.full((patches.size(0), patch_embeddings.size(1)), CFG.LABEL_MASK, device=device)
                    
                    # Keep text labels as-is for loss computation
                    text_labels = tokenized["input_ids"].clone()
                    
                    # Also mask padding tokens in text
                    text_labels[tokenized["attention_mask"] == 0] = CFG.LABEL_MASK

                    # Concatenate all labels - only text portion will contribute to loss
                    ...
                    print(f"DEBUG - Prompt labels (all -100): {prompt_labels[0]}")
                    print(f"DEBUG - Patch labels (all -100): {patch_labels[0]}")
                    print(f"DEBUG - Text labels (actual tokens): {text_labels[0]}")
                    print(f"DEBUG - Number of non-masked text tokens: {(text_labels != CFG.LABEL_MASK).sum()}")
                    print("="*80)

                    # Forward pass - loss will only be computed on text tokens due to masking
                ...
                def generate(self, patches: torch.Tensor, generator_kwargs: dict[str, Union[int, float]]):
                    ...
                    # Create embeddings for generation (prompt + image patches only)
                    embeddings = torch.cat([
                        self.prompt_embeddings.to(device).expand(patches.size(0), -1, -1),
                        patch_embeddings,
                    ], dim=1)
            ...
            class MaskedLossLightningModule(LIGHTNING.LightningModule):
                def __init__(self, model: MaskedLossModel):
                    super().__init__()
                    self.model = model
                    self.lr = 2e-4
                ...
                def training_step(self, batch, batch_idx):
                    ...
                    print(f"Batch {batch_idx} - Loss: {out.loss.item():.4f}")
            ...
            model = MaskedLossModel(image_model, language_model1, tokenizer)
            lightning_module = MaskedLossLightningModule(model)
            
            print("STARTING TRAINING WITH MASKED LOSS (TEXT TOKENS ONLY)...")
            print("*** LOSS WILL ONLY BE COMPUTED ON TEXT TOKENS - PROMPT AND IMAGE PATCHES ARE MASKED ***")
            print("="*100)
            ...
            patches_batch = np.array(patches[-10:])  # Last 10 samples
            ...
            for i in tqdm(range(generated_ids.shape[0])):
                # Calculate where new tokens start (after prompt + image patches)
                prompt_length = model.prompt_embeddings.shape[1]
                # Calculate patch length based on image features
                sample_patch = patches_batch[0:1]  # Single sample for calculation
                with torch.no_grad():
                    sample_features = model.image_model.forward_features(sample_patch.to(device))
                    sample_features = einops.rearrange(sample_features, "bs dim w h -> bs (w h) dim")
                    patch_length = sample_features.shape[1]
                ...
                print(f"Original text: {texts[-(10-i)]}")  # Show original text for comparison
                print("-" * 80)
    ...
    print("MASKED LOSS TRAINING WORKFLOW FINISHED SUCCESSFULLY!")
    print("="*100)
